...
// Define which slides should use light text (dark backgrounds)
const lightTextSlides = [0, 2, 4] // Adjust these indices based on your artwork brightness
...
export default function HeroSlideshow() {
  ...
  const isLightText = lightTextSlides.includes(currentIndex)
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Image
            ...
            fill
            className={`object-cover ${isLightText ? "brightness-75" : "brightness-100"}`}
            priority
          />
          <div className={`absolute inset-0 ${isLightText ? "bg-black/30" : "bg-white/30"}`} />
          ...
          <motion.div
            ...
            className={`absolute bottom-8 right-8 ${
              isLightText ? "text-white bg-black/40" : "text-black bg-white/40"
            } backdrop-blur-sm p-6 rounded-lg max-w-md`}
          >
            <h2 className="text-2xl font-semibold mb-2">{artworks[currentIndex].title}</h2>
            <p className="text-lg mb-4">by {artworks[currentIndex].artist}</p>
            <Button 
              variant="outline" 
              asChild 
              className={`w-full ${
                isLightText 
                  ? "bg-black/10 text-white hover:bg-black/20 border-white" 
                  : "bg-white/10 text-black hover:bg-white/20 border-black"
              }`}
            >
