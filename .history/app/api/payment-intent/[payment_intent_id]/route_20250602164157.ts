...
export async function GET(request: Request) {
  ...
  try {
    ...
    const purchaseDetails = {
      ...
      customerName: paymentIntent.shipping?.name || paymentIntent.billing_details?.name,
      customerEmail: paymentIntent.receipt_email || paymentIntent.billing_details?.email,
      customerAddress: paymentIntent.shipping?.address?.line1 || paymentIntent.billing_details?.address?.line1,
    };
