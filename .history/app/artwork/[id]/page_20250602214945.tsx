...
interface PurchaseInfo {
  ...
  email: string;
  address: string;
  phone?: string; // Add phone as an optional string
}
                    ...
                    {!showPayment ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 h-full divide-x divide-border">
                        ...
                        <div className="p-6 md:p-8 bg-muted dark:bg-gray-800 space-y-6 flex flex-col justify-between">
                          <div className="space-y-6">
                            ...
                            <div className="space-y-4 text-sm">
                              <div>
                                <div className="text-muted-foreground">Artwork:</div>
                              ...
                              {purchaseInfo && (
                                <>
                                  <div>
                                    ...
                                    <div className="font-medium">{purchaseInfo.name}</div>
                                  </div>
                                  <div>
                                    <div className="text-muted-foreground">Email:</div>
                                    <div className="font-medium">{purchaseInfo.email}</div>
                                  </div>
                                  {purchaseInfo.phone && (
                                    <div>
                                      <div className="text-muted-foreground">Phone:</div>
                                      <div className="font-medium">{purchaseInfo.phone}</div>
                                    </div>
                                  )}
                                  <div>
                                    <div className="text-muted-foreground">Delivery Address:</div>
                                    <div className="font-medium">{purchaseInfo.address}</div>
                                  </div>
                                </>
                              )}
                            </div>
                        ...
                        <div className="bg-white p-6 md:p-8">
                          ...
                          <p className="text-muted-foreground text-sm mb-6">Enter your payment information to complete the purchase</p>
                          <Elements options={options} stripe={stripePromise}>
                            <CheckoutForm 
                              artworkId={artwork.id} 
                              clientSecret={clientSecret} 
                              buyerEmail={purchaseInfo?.email || ''} // Provide fallback
                              buyerName={purchaseInfo?.name || ''} // Provide fallback
                            />
                          </Elements>
                        </div>
                      </div>
