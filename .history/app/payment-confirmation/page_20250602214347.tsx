...
export default function PaymentConfirmationPage() {
  ...
  useEffect(() => {
    ...
    if (!payment_intent || !payment_intent_client_secret) {
      setStatus('error');
      setError('Payment details not found. Missing payment_intent.');
      return;
    }
    ...
    if (redirect_status === 'succeeded') {
      ...
      if (purchaseDetails) {
        const { artworkId, buyerEmail, buyerName } = JSON.parse(purchaseDetails);

        // Send confirmation emails via API route
        fetch('/api/send-purchase-emails', {
          method: 'POST',
          headers: {
